...
export default function housePage() {
  ...
  const houseTypeFilters = [
    { label: '1BHK', value: '1BHK', icon: FaHome },
    { label: '2BHK', value: '2BHK', icon: FaHome },
    { label: '3BHK', value: '3BHK', icon: FaHome }
  ];
  ...
  const fetchHouses = async () => {
    ...
    try {
      ...
      const url = hasFilters 
        ? `${API}/house/filter-for-user?${params.toString()}`
        : `${API}/house-for-user`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-4 hidden md:block">
            ...
            <p className="text-base md:text-lg text-gray-600 mb-4 max-w-2xl mx-auto">
              Discover comfortable and affordable house across India
            </p>
          </div>
          ...
          <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-lg p-3 md:p-5 border border-gray-100">
            <div className="flex flex-wrap items-center gap-2 md:gap-3">
              ...
              <div className="relative flex-1 min-w-[140px] basis-[calc(50%-0.25rem)] md:basis-auto">
                <FaHome className="absolute left-2.5 top-1/2 -translate-y-1/2 text-orange-600 text-xs pointer-events-none z-10" />
          ...
          <div className="max-w-6xl mx-auto mt-3 flex justify-center">
            <button
              onClick={() => {
                ...
                if (areaName) {
                  router.push(`/house/map-search?area=${encodeURIComponent(areaName)}`);
                } else {
                  router.push('/house/map-search');
                }
              }}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div>
              <h2 className="text-xl md:text-3xl font-bold text-gray-900 flex items-center gap-2 md:gap-3">
               {/* <span className="text-orange-600">{houses.length}</span>  */}
                <span>House</span>
              </h2>
              <p className="text-xs md:text-sm text-gray-600 mt-1">
                {hasActiveFilters ? 'Filtered results' : 'All available houses'}
              </p>
            </div>
            {/* <button
              onClick={fetchHouses}
          ...
          {!loading && houses.length === 0 && (
            <div className="text-center py-20">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-3">No Houses Found</h3>
              <p className="text-gray-600 mb-8 max-w-md mx-auto">
                {hasActiveFilters 
                  ? 'Try adjusting your filters to see more results' 
                  : 'No houses are currently available'}
              </p>
              {hasActiveFilters && (
                <button 
                  onClick={clearAllFilters}
