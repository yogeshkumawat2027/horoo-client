...
export default function FlatPage() {
  ...
  const flatTypeFilters = [
    { label: '1BHK', value: '1BHK', icon: FaBed },
    { label: '2BHK', value: '2BHK', icon: FaUsers },
    { label: '3BHK', value: '3BHK', icon: FaUsers }
  ];

  const availableForFilters = [
    { label: 'Boys', value: 'Boys', icon: FaUsers },
    { label: 'Girls', value: 'Girls', icon: FaUserFriends },
    { label: 'Family', value: 'Family', icon: FaHome }
  ];
  ...
  // Fetch flats when filters change
  useEffect(() => {
    fetchFlats();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filters]);
  ...
  const fetchFlats = async () => {
    ...
    try {
      ...
      const url = hasFilters 
        ? `${API}/flats/filter-for-user?${params.toString()}`
        : `${API}/flats-for-user`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-4 hidden md:block">
            ...
            <p className="text-base md:text-lg text-gray-600 mb-4 max-w-2xl mx-auto">
              Discover comfortable and affordable flats across India
            </p>
          </div>
          ...
          <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-lg p-3 md:p-5 border border-gray-100">
            <div className="flex flex-wrap items-center gap-2 md:gap-3">
              ...
              <div className="relative flex-1 min-w-[140px] basis-[calc(50%-0.25rem)] md:basis-auto">
                <FaBed className="absolute left-2.5 top-1/2 -translate-y-1/2 text-orange-600 text-xs pointer-events-none z-10" />
                <FaChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs pointer-events-none z-10" />
                <select
                  value={filters.flatType}
                  onChange={(e) => setFilters(prev => ({ ...prev, flatType: e.target.value }))}
                ...
                >
                  ...
                  {flatTypeFilters.map((filter) => (
                    <option key={filter.value} value={filter.value}>{filter.label} Flat</option>
                  ))}
                </select>
              </div>
              ...
              <div className="relative flex-1 min-w-[140px] basis-[calc(50%-0.25rem)] md:basis-auto">
                <FaUsers className="absolute left-2.5 top-1/2 -translate-y-1/2 text-orange-600 text-xs pointer-events-none z-10" />
                <FaChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs pointer-events-none z-10" />
                <select
                  value={filters.availableFor}
                  onChange={(e) => setFilters(prev => ({ ...prev, availableFor: e.target.value }))}
                  className="w-full pl-8 pr-7 py-2 md:py-2.5 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white text-gray-900 text-xs md:text-sm font-medium transition-all cursor-pointer hover:border-orange-400 appearance-none"
                >
                  <option value="">Available For</option>
                  {availableForFilters.map((filter) => (
          ...
          <div className="max-w-6xl mx-auto mt-3 flex justify-center">
            <button
              onClick={() => {
                ...
                if (areaName) {
                  router.push(`/flats/map-search?area=${encodeURIComponent(areaName)}`);
                } else {
                  router.push('/flats/map-search');
                }
              }}
          ...
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div>
              <h2 className="text-xl md:text-3xl font-bold text-gray-900 flex items-center gap-2 md:gap-3">
               {/* <span className="text-orange-600">{flats.length}</span>  */}
                <span>Flats</span>
              </h2>
              <p className="text-xs md:text-sm text-gray-600 mt-1">
                {hasActiveFilters ? 'Filtered results' : 'All available flats'}
              </p>
            </div>
            {/* <button
              onClick={fetchFlats}
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-600 mb-8 max-w-md mx-auto">
                {hasActiveFilters 
                  ...
                  : 'No flats are currently available'}
              </p>
              {hasActiveFilters && (
                <button 
                  onClick={clearAllFilters}
