...
import HostelCard from '@/components/Home/cards/HostelCard';
...
export default function hostelPage() {
  ...
  const roomTypeFilters = [
    { label: 'Single', value: 'Single', icon: FaBed },
    { label: 'Double', value: 'Double', icon: FaUsers },
    { label: 'Triple', value: 'Triple', icon: FaUsers }
  ];
  ...
  // Fetch hostels when filters change
  useEffect(() => {
    fetchHostels();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filters]);
  ...
  const fetchHostels = async () => {
    ...
    try {
      ...
      const url = hasFilters 
        ? `${API}/hostels/filter-for-user?${params.toString()}`
        : `${API}/hostels-for-user`;
              ...
              <div className="relative flex-1 min-w-[140px] basis-[calc(50%-0.25rem)] md:basis-auto">
                ...
                >
                  ...
                  {roomTypeFilters.map((filter) => (
                    <option key={filter.value} value={filter.value}>{filter.label} Room</option>
                  ))}
                </select>
              </div>
          ...
          <div className="max-w-6xl mx-auto mt-3 flex justify-center">
            <button
              onClick={() => {
                ...
                if (areaName) {
                  router.push(`/hostels/map-search?area=${encodeURIComponent(areaName)}`);
                } else {
                  router.push('/hostels/map-search');
                }
              }}
          ...
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div>
              <h2 className="text-xl md:text-3xl font-bold text-gray-900 flex items-center gap-2 md:gap-3">
               {/* <span className="text-orange-600">{hostels.length}</span>  */}
                <span>Hostels</span>
              </h2>
              <p className="text-xs md:text-sm text-gray-600 mt-1">
                {hasActiveFilters ? 'Filtered results' : 'All available hostels'}
              </p>
            </div>
            {/* <button
